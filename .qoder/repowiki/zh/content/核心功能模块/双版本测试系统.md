...
- [app/test/page.tsx](file://app/test/page.tsx)
- [app/test/quiz/page.tsx](file://app/test/quiz/page.tsx)
- [app/test-ticket/page.tsx](file://app/test-ticket/page.tsx)
...
- [data/questions-test.ts](file://data/questions-test.ts)
...
## 更新摘要
**变更内容**
- 新增测试版双版本测试系统，显著增强了精简版和完整版测试模式的选择机制
- 引入全新的测试版页面结构和交互体验
- 增强了测试流程的灵活性和用户体验
- 新增电影票根撕纸效果测试功能
- 更新测试策略和使用方法
...
FBTI（Film Buff Type Indicator）是一个基于电影人格类型的双版本测试系统。该项目灵感来源于MBTI性格测试，专门为电影爱好者设计，通过精心设计的问题来识别用户的电影人格类型。

### 主要特性

- **双版本测试**：提供快速测试（24道题，约2分钟）和完整版测试（80+道题，约5分钟）
- **四大维度分析**：感知模式、探索方式、叙事引力、影调趋向
- **隐藏属性系统**：时间穿越者、形式感应器、文化通行证
- **个性化推荐**：基于用户类型生成导演和电影推荐
- **分享功能**：支持生成电影人格分享卡片
- **开发测试自动化**：内置脚本支持快速测试和策略验证
- **测试版增强**：全新的测试模式选择机制和优化体验
...
A --> A2[page.tsx]
A --> A3[test/page.tsx]
A --> A4[test/quiz/page.tsx]
A --> A5[test-ticket/page.tsx]
A --> A6[quiz/page.tsx]
A --> A7[result/page.tsx]
A --> A8[encyclopedia/page.tsx]
end
subgraph "工具层"
...
C[data/] --> C1[types.ts]
C --> C2[questions.ts]
C --> C3[questions-test.ts]
C --> C4[directorsMeta.ts]
end
subgraph "开发工具"
...
- [utils/calculator.ts:1-644](file://utils/calculator.ts#L1-L644)
- [data/types.ts:1-428](file://data/types.ts#L1-L428)
- [data/questions-test.ts:1-1896](file://data/questions-test.ts#L1-L1896)
...
- **主页**：提供快速测试和完整版测试入口
- **测试版主页**：新增的测试模式选择页面，提供优化的测试体验
- **测试问答页面**：处理测试题目的交互，支持测试模式标记
- **结果页面**：展示电影人格分析和个性化推荐
...
- [app/test/page.tsx:1-163](file://app/test/page.tsx#L1-L163)
- [app/test/quiz/page.tsx:1-567](file://app/test/quiz/page.tsx#L1-L567)
- [app/test-ticket/page.tsx:1-311](file://app/test-ticket/page.tsx#L1-L311)
- [scripts/quick-test.js:1-154](file://scripts/quick-test.js#L1-L154)
- [scripts/quick-test-simple.js:1-64](file://scripts/quick-test-simple.js#L1-L64)
...
participant TP as 测试版主页
participant Q as 测试问答页面
...
**图表来源**
- [app/test/quiz/page.tsx:29-34](file://app/test/quiz/page.tsx#L29-L34)
...
**图表来源**
- [data/questions-test.ts:33-42](file://data/questions-test.ts#L33-L42)
...
FBTI项目引入了全新的测试版双版本测试系统，显著增强了用户体验和测试精度：

#### 测试版主页 (TestLandingPage)

**主要功能**：
- 提供测试版和正式版两种测试入口
- 精简版测试：24道精选题目，约2分钟完成
- 完整版测试：80+道题，约5分钟完成
- 测试模式标记：区分测试版和正式版数据流
...
- PW维度：4道精选题目
- LD维度：7道精选题目
- 观影画像题：4道（影厅偏好、观影环境、观影社交、购票习惯）

**测试版特色**：
- 测试模式标记：`sessionStorage.setItem("fbti_test_mode", "true")`
- 优化的题目体验：针对测试目的的精选题目组合
- 快速反馈机制：缩短测试时间，提高用户体验

#### 测试版问答页面 (TestQuizPage)

**主要增强**：
- 测试模式支持：检测并处理测试版数据流
- 优化的进度管理：支持测试版题目ID过滤
- 测试版数据清理：测试完成后自动清理临时数据
- 测试版进度保持：支持测试过程中的数据持久化

**测试版数据管理**：
- 从sessionStorage读取测试版题目ID
- 支持测试版和正式版数据的分离
- 测试完成后自动清理测试版临时数据
...
L[Test Ticket Page]
M[Quiz Page]
N[Questions Test Data]
O[Questions Data]
end
subgraph "开发工具"
P[Quick Test Script]
Q[Simple Test Script]
end
subgraph "数据模块"
R[Types Data]
S[Directors Meta]
T[Questions Test Data]
U[Questions Data]
end
A --> F
B --> F
...
- [app/test-ticket/page.tsx:100-125](file://app/test-ticket/page.tsx#L100-L125)
...
5. **测试版增强**：全新的测试模式选择和优化体验

该系统为电影爱好者提供了一个深入了解自己观影偏好的工具，同时也为电影推荐和营销提供了有价值的参考数据。新增的测试版双版本测试系统进一步提升了系统的开发效率和质量保证能力，为未来的功能扩展奠定了坚实基础。