...
## 更新摘要
**变更内容**
- 新增多选题支持（multiSelect），支持限制最大选择数
- 重构问题格式，将部分 binary 问题升级为 multi 问题
- 集成 TMDB 图片资源，支持多种布局类型（single、split、grid3、grid4）
- 增强隐藏属性系统，支持按类型分类的偏好（delta）
- 新增双版本测试系统（快速版24题和完整版40题）
...
**更新** 本版本文档已更新以支持新增的多选题支持、TMDB集成和增强的隐藏属性系统。
...
QT["questions-test.ts<br/>测试问题数据"]
...
TP["app/test/quiz/page.tsx<br/>测试页面"]
...
- [questions-test.ts:1-42](file://data/questions-test.ts#L1-L42)
...
participant QT as "测试页面<br/>test/quiz/page.tsx"
...
- attribute：隐藏属性标识，取值为 α、β、γ、δ 四种之一
  ...
  - γ：文化通行证（偏好国际/独立）
  - δ：类型偏好（按类型累加）
- genre：可选的类型标签，当前支持 horror、comedy、scifi、crime、romance、action
...
- multiSelect 类型（多选且限制数量）
  - 示例路径：[multiSelect 示例:363-395](file://data/questions.ts#L363-L395)
...
- [questions.ts:363-395](file://data/questions.ts#L363-L395)
...
**更新** 新增的多选题支持显著提升了问题的丰富性和用户的选择灵活性，TMDB集成使得问题展示更加生动直观。增强的隐藏属性系统通过按类型分类的偏好（delta）为个性化推荐提供了更精确的数据基础。新增的双版本测试系统显著提升了用户体验，快速版测试满足了用户的即时需求，完整版测试提供了更深入的分析。电影个性分析系统通过 Q50-Q53 问题生成个性化的电影推荐，"电影社会学家"彩蛋增加了趣味性和互动性。
